	...
	<view class="container">
		<view class="top flex_between">
			<view class="search flex_center">
				...
				<input type="text" v-model="searchKey" confirm-type="search" @confirm="search" placeholder="请输入要搜索的内容" placeholder-style="color:#CBCBCB;font-size:28rpx;line-height:28rpx"/>
			</view>
			<view><text class="fz-12">商家总数</text><text class="fz-24">{{total}}</text></view>
		</view>
		...
		<scroll-view scroll-y="true" class="list" @scrolltolower="reachBottom">
			<view class="item flex_between" v-for="item in list" :key="item.id" @click="goCheck(item.id)">
	...
	export default{
		data() {
			return {
				...
				tab:[
					{id:1,text:'全部',value:''},
					{id:2,text:'审核通过',value:'normal'},
					{id:3,text:'待审核',value:'audit'},
					{id:4,text:'驳回',value:'audit_failtrue'}
				],
				page:0,
				list:[],
				total:0,
		...
		methods:{
			...
			getList(){ //获取区域商家列表
				this.status = 'loading';
				this.$fly.get(`/merchant/list?status=${this.current}&page=${this.page}&userId=${this.$store.state.userInfo.id}&key=${this.searchKey}`)
				.then(res=>{
			...
			goCheck(id){ //商家审核
				uni.navigateTo({
					url:`/operationPages/businessApproval/index?id=${id}`
				})
			},
			reachBottom(){ //触底加载
