	...
	<view class="bg">
		<view class="consume">
			<view class="title">
				...
				<view class="money"><text class="fz-12">¥</text><text class="fz-20">{{queryInfo.money}}</text></view>
			</view>
...
<script>
	export default{
		...
		methods:{
			getCoupon(){ //获取优惠券列表
				this.status = 'loading';
				this.$fly.get(`/coupon/list?status=AVAILABLE&userId=${this.$store.state.userInfo.id}&page=${this.page}&lockMerchantId=${this.queryInfo.merchantId}`)
				.then(res=>{
					// console.log(res)
					if(res.code == 0){
			...
			changeNeedPay(){ //需支付金额
				let sum = this.queryInfo.money, //消费金额
				fee = this.queryInfo.payCommission,
